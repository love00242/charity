...
<template>
    <div
        :class="[isPC ? 'pcBg' : isLeft ? 'leftBg' : 'rightBg items-end', 'flex justify-end flex-col bg-[length:100%_100%] relative']">
        <!-- 手機 -->
        <template v-if="!isPC">
            <div :class="['w-full h-full absolute z-0 ', isLeft ? 'bg-[#e4e4e4]/20' : 'bg-[#3f3f3f]/40']" />
            <p
                :class="['font-bold text-2xl text-center mb-[20%] mx-auto px-2.5 z-[1] whitespace-pre-line md:mb-[10%]', { 'text-white': !isLeft }]">
                {{ content.text }}</p>
        ...
        <template v-else>
            ...
            <!-- 文字 -->
            <p
                :class="['font-bold text-[39px] text-center z-[1] whitespace-pre-line pr-[50%]', { 'text-white pl-[50%] whitespace-pre-wrap pr-[unset]': !isLeft }]">
                {{ content.text }}</p>
